...
🔸 **LEFT OUTER JOIN (רק מי שלא מצא מונית)**

* הצג רק את הנוסעים שאין להם taxi\_id תואם

🔸 **FULL OUTER JOIN (סימולציה)**
...
* הסקריפט צריך ליצור את הטבלאות, להכניס את הנתונים, להריץ את כל השאילתות מהסעיף הקודם, ולהדפיס את התוצאות למסך
* כתוב סקריפט פייתון שמשתמש בספריית `sqlite3`
